...
const context = createComponentState({ state: () => ({
  ...
  setScrollTop(scrollTopPer: number, smooth = false) {
    ...
    const scrollPx = ratio * (this.state.refContent.scrollHeight - this.state.refContent.clientHeight + 1)
    if (!smooth) {
  ...
  setScrollLeft(scrollLeftPer: number, smooth = false) {
    ...
    const scrollPx = ratio * (this.state.refContent.scrollWidth - this.state.refContent.clientWidth + 1)
    if (!smooth) {
