...
function ContentCore(props: ContentProps) {
  ...
  return (
    <div
      ...
      ref={mergeRefs(localProps.ref, r => ref = r)}
      style={{
...
export function Panel(props: ContentProps) {
  ...
  return (
    <Show when={state.active === props.key || state.tryOpen === props.key || state.tryClose === props.key}>
      <ContentCore {...props} />
    </Show>
  )
}
