...
function Dialog(props: {
  ...
  return (
    <ModalCore preventScroll={true}>
      <ModalCore.Trigger>
        <button class="bg-amber p-2" ref={triggerRef}>Trigger</button>
      </ModalCore.Trigger>
      <ModalCore.Portal>
          ...
          {(state, actions) => {
            watch(() => state.status, (status) => {
              if (status === 'opening') {
                ...
                contentRef.style.setProperty('--origin-x', `${(centerTrigger.x - centerContent.x) + contentRect.width / 2}px`)
                contentRef.style.setProperty('--origin-y', `${(centerTrigger.y - centerContent.y) + contentRect.height / 2}px`)
              }
            })
            return (
              <div
              ...
              >
                <DraggableCore>
                  <DraggableCore.Content>
                    <div
                      class="p-2 bg-gray-4 flex flex-col gap-3 "
                    >
                      ...
                      <button class="bg-red p-2" onClick={() => ModalCore.closeAll()}>
                        Close All
                      </button>
                    </div>
                  </DraggableCore.Content>
